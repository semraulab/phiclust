#' Plot all phiclusts
#'
#' plot of all phiclusts obtained per cluster
#'
...
plot_all_phiclusts <- function(measure_obj){
  ...
  p <- ggplot(measure_obj$all_info, aes(x = .data$celltype, y = .data$phiclust, colour = .data$singular_value)) +
    geom_point(size = 2) + theme_light(base_size = 12) + xlab("") +
    ylab(expression(phi["clust"])) + theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
